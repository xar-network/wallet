...
class RewardsCSDT extends Component {
  ...
  constructor(props) {
    ...
    this.calculateDelegationRewardsBalance = this.calculateDelegationRewardsBalance.bind(this)
    this.calculateDelegationRewards = this.calculateDelegationRewards.bind(this)
  ...
  calculateDelegationRewardsBalance(validatorAddress) {
    const { delegationRewards } = this.props

    let reward = 0

    if(delegationRewards && delegationRewards.rewards && delegationRewards.rewards.length > 0) {
      let validator = delegationRewards.rewards.filter((reward) => {
  ...
  render() {
    ...
    const delegatedBalance = this.calculateDelegationRewards()
    const currentValidatoroBalance = this.calculateDelegationRewardsBalance(recipient)
    ...
    return (
      <Grid
        ...
        <Grid item xs={10} className={classes.header}>
          <Typography variant="h2" className={ classes.title }>Get Delegation Rewards</Typography>
        </Grid>
        ...
        <Grid item xs={12} className={ classes.sepperate }>
          <FormControl variant="outlined" className={classes.textField}>
            <Typography variant="body1">Which validator would you like to receive rewards from?</Typography>
            <Select
            ...
            >
              {
                (validators && validators.length > 0) && validators.map((validator) => {
                  return (
        ...
        <Grid item xs={12} className={ classes.sepperate }>
          ...
          <Typography variant="body1" className={ classes.infoTitle }>Total pending rewards</Typography>
          <Typography variant="h3" className={ classes.infoValue }>{ delegatedBalance + ' ' + generatedDenom }</Typography>
          <Typography variant="body1" className={ classes.infoTitle }>Pending rewards at validator</Typography>
...
export default withRouter(connect(mapStateToProps)(withStyles(styles)(RewardsCSDT)))
