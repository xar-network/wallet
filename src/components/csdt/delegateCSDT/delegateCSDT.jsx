...
class DelegateCSDT extends Component {
    ...
    return (
      <Grid
        ...
        <Grid item xs={10} className={classes.header}>
          <Typography variant="h2" className={ classes.title }>Delegate {generatedDenom}</Typography>
        </Grid>
        <Grid item xs={2} align="right">
          <CloseIcon onClick={onClose} className={ classes.closeButton }/>
        </Grid>
        <Grid item xs={12} className={ classes.sepperate }>
          <Typography variant="body1">How much {generatedDenom} would you like to delegate?</Typography>
          <TextField
...
export default withRouter(connect(mapStateToProps)(withStyles(styles)(DelegateCSDT)))
